...
{
    for(PlaceableObject* po : children())
        unparent_child(po);
}
...
std::list<PlaceableObject *> PlaceableObject::children(void) const
{
    ...
    for (PlugBase *conn : _world_transform.outgoing_connections())
    {
        if (conn->name() == "Parent_Transform")
            children.push_back(checked_static_cast<PlaceableObject *>(conn->owner()));
    }
